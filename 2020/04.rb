...
input.each do |passport|
  valid += 1 if KEYS.all? { |s| passport.key? s }
end
...
input.each do |passport|
  if KEYS.all? { |s| passport.key? s }
    if passport['byr'][/^\d{4}$/] && passport['byr'].to_i >= 1920 && passport['byr'].to_i <= 2002 &&
    passport['iyr'][/^\d{4}$/] && passport['iyr'].to_i >= 2010 && passport['iyr'].to_i <= 2020 &&
    passport['eyr'][/^\d{4}$/] && passport['eyr'].to_i >= 2020 && passport['eyr'].to_i <= 2030 &&
    (h, u = passport['hgt'].scan(/^(\d+)(cm|in)$/)[0]) && u && (u["in"] ? h.to_i >= 59 && h.to_i <= 76 : h.to_i >= 150 && h.to_i <= 193) &&
    passport['hcl'][/^#\h{6}$/] &&
